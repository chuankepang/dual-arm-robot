    ...
    while True:
        ...
        if success:
            ...
            if len(lmslist) > 0:
                ...
                pose = Pose()  #创建 msg 对象
                #msg2 = Int32()  #创建 msg 对象
                pose.position.x = sum_x
                pose.position.y = sum_y
                pose.position.z = 100
                ...
                #pub.publish(msg2)
                #rate.sleep()
                rospy.loginfo("\n position \n x : %f \n y : %f \n z : %f \n orientation \n x : %f \n y : %f \n z : %f \n w : %f", pose.position.x, pose.position.y, pose.position.z, pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w)
                #rospy.loginfo("写出的数据:%s",msg2)
                count += 1
